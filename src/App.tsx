...
const MarkdownContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (redirectPath) {
      ...
      const normalizedPath = redirectPath.startsWith('/') ? redirectPath.substring(1) : redirectPath;
  ...
  const fetchFileTree = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:3500/api/folder-structure');
      ...
      setError('Failed to load file tree. Please make sure the server is running.');
      setLoading(false);
    }
  };
  ...
  const handleFileSelect = async (path: string) => {
    try {
      ...
      const response = await fetch(`http://localhost:3500/api/file?path=${normalizedPath}`);
      if (!response.ok) {
